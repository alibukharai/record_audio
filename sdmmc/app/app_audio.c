...
static esp_err_t audio_record_stop()
{
    ...
    ESP_LOGI(TAG, "### record Stop, %d, %d K", \
             record_total_len, \
...
void sr_handler_task(void *pvParam)
{
    ...
    while (true) {
        ...
        if (ESP_MN_STATE_TIMEOUT == result.state) {
            ...
            size_t len;
            audio_mp3_load("/spiffs/waitPleasechinese.mp3", &len);
            if (len) {
            ...
            ESP_LOGE(TAG, "[Start] start_openai, timestamp:%d", starttime);
            if (WIFI_STATUS_CONNECTED_OK == wifi_connected_already()) {
                start_openai((uint8_t *)record_audio_buffer, record_total_len);
            }
            ESP_LOGE(TAG, "[End] start_openai, +offset:%d", esp_log_timestamp() - starttime);
            memory_monitor();
            continue;
        }
